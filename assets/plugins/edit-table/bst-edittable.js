...
class BSTable {
    ...
    constructor(tableId, options) {
        ...
        var defaults = {
            ...
            onColEdit: function() {}, // Called after editing (accept button clicked)
    ...
    _colActionsModeNormal(button) {
        $(button).parent().find('#cbAcep').hide();
        $(button).parent().find('#cbCanc').hide();
        $(button).parent().find('#cbEdit').show();
        let $currentRow = $(button).parents('tr'); // get the row
        $currentRow.attr('data-status', ''); // remove editing status
    }
    _colActionsModeEdit(button) {
        $(button).parent().find('#cbAcep').show();
        $(button).parent().find('#cbCanc').show();
        $(button).parent().find('#cbEdit').hide();
        let $currentRow = $(button).parents('tr'); // get the row
        $currentRow.attr('data-status', 'editing'); // indicate the editing status
    }
    ...
    _colEdit(button) {
        // Indicate user is editing the row
        let colType = $(button).attr('data-col-edit');
        let $currentCol = this.table.find('td.'+colType);
        for(let key in $currentCol) {
            key = parseInt(key);
            if(isNaN(key)) {break;}

            let $cell = $($currentCol[key]);

            let content = $cell.html();
            let div = '<div style="display: none;">'+content+'</div>';
            let input = '<input class="form-control input-sm"  data-original-value="' + content + '" value="' + content + '">';
            $cell.html(div + input); // set content
        }
        this._colActionsModeEdit(button);
    }
    ...
    _colAccept(button) {
        ...
        let colType = $(button).attr('data-col-edit');
        let $currentCol = this.table.find('td.'+colType);
        let subjectId = this.table[0].getAttribute('data-subjectid');
        let data = {};
        ...
        for(let key in $currentCol) {
            ...
            let $cell = $($currentCol[key]);
            let studid = $cell.parents('tr')[0].getAttribute('data-studid');
            let value = $cell.find('input')[0].value;

            data[studid] = value;

            $cell.html(value); // set content

        }

        this.options.onColEdit(data, colType, subjectId);
        this._colActionsModeNormal(button);
        return ;
        if (!this.currentlyEditingRow($currentRow)) return; // not currently editing, return
    ...
    _colCancel(button) {
        ...
        for(let key in $currentCol) {
            ...
            let content = $cell.find('div').html();
            let input = content;
            $cell.html(input); // set content
        }
        this._colActionsModeNormal(button);
    }
    ...
    _rowAccept(button) {
        // Accept the changes to the row
        let $currentRow = $(button).parents('tr'); // access the row
        let studid = $(button).parents('tr').attr('data-studid'); // access the row
        // console.log($currentRow);
    ...
    _addOnClickEventsToActions() {
        ...
        this.table.find('thead #cbEdit').each(function() {
            let button = this;
            button.onclick = function() { _this._colEdit(button) }
        });
        this.table.find('thead #cbAcep').each(function() {
            let button = this;
            button.onclick = function() { _this._colAccept(button) }
        });
        this.table.find('thead #cbCanc').each(function() {
            let button = this;
            button.onclick = function() { _this._colCancel(button) }
        });
